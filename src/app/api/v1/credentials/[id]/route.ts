...
export const GET = apiHandler(async (req: NextRequest, context: { params: Promise<{ id: string }> }) => {
  ...
  const credential = await db.getCredentialById(id, authUser.userId);
...
export const PATCH = apiHandler(async (req: NextRequest, context: { params: Promise<{ id: string }> }) => {
  ...
  const updated = await db.updateCredential(id, body, authUser.userId);
  if (!updated) throw new NotFoundError('Credential not found or permission denied');
...
export const DELETE = apiHandler(async (req: NextRequest, context: { params: Promise<{ id: string }> }) => {
  ...
  const { id } = await context.params;
  const success = await db.deleteCredential(id, authUser.userId);
  if (!success) throw new NotFoundError('Credential not found or permission denied');
